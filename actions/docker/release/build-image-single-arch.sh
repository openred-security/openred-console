...
# Docker build
docker build --build-arg VERSION=$VERSION --build-arg BUILD_DATE=`date -u +%Y-%m-%dT%H:%M:%SZ` --build-arg NOTES=$NOTES -f $DOCKERFILE $DIR -t openred/$PRODUCT:$VERSION
docker tag openred/$PRODUCT:$VERSION openred/$PRODUCT:latest
